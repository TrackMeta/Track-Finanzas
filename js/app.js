...
async function init() {
  ...
  // 2. Si no hay sesión, hacer login automático con las credenciales de config.js
  if (!user && MY_EMAIL && MY_PASSWORD) {
    const { data } = await Auth.signInWithPassword(MY_EMAIL, MY_PASSWORD);
    user = data?.user ?? null;
  }
  ...
  if (user) {
    ...
    // Solo si todo falla mostrar mensaje de error
    $('screen-login').classList.remove('hidden');
    $('screen-login').classList.add('active');
    $('form-magic-link').querySelector('button').textContent = 'No se pudo conectar. Revisa config.js';
    $('form-magic-link').querySelector('button').disabled = true;
  }
